...
public class FirmExtractor extends AbstractJdbcExtractor<Firm> {
    @Override
    public List<Firm> extractData(ResultSet rs) throws SQLException, DataAccessException {
        Map<EntityId, Firm> firmIdMap = new LinkedHashMap<>();
        while (rs.next()) {
            Firm firm = new FirmMapper(FirmJdbcRepo.TABLE_ALIAS).createEntityFromResultSet(rs);
            firmIdMap.putIfAbsent(firm.getId(), firm);
        }
        return firmIdMap.values().stream().toList();
    }
}
